...
for i in range(folds):
    ...
    # Preparing x test vector
    x_test_vector = x_vector[i*part:i*part+part]

    # Preparing x train vector
    x_train_first = x_vector[0:i*part]
    x_train_last = x_vector[i*part+part:]
    x_train_vector = np.concatenate((x_train_first, x_train_last))

    if Config.get(scenario, 'wordshape') == 'True':
        # Preparing x test vector
        x_ws_test_vector = x_ws[i*part:i*part+part]

        # Preparing x train vector
        x_ws_train_first = x_ws[0:i*part]
        x_ws_train_last = x_ws[i*part+part:]
        x_ws_train_vector = np.concatenate((x_ws_train_first, x_ws_train_last))

    # Preparing y test vector
    y_test_vector = y_vector[i*part:i*part+part]

    # Preparing y train vector
    y_train_first = y_vector[0:i*part]
    y_train_last = y_vector[i*part+part:]
    y_train_vector = np.concatenate((y_train_first, y_train_last))

    # Preparing mask test vector
    mask_test_vector = mask_vector[i*part:i*part+part]

    # Preparing mask train vector
    mask_train_first = mask_vector[0:i*part]
    mask_train_last = mask_vector[i*part+part:]
    mask_train_vector = np.concatenate((mask_train_first, mask_train_last))

    if Config.get(scenario, 'wordshape') == 'True':
        (f1_org, f1_loc, f1_per) = train_and_evaluate_model(model, x_train_vector, y_train_vector, mask_train_vector,
                                                            x_test_vector, y_test_vector, mask_test_vector, label_dict, x_ws_train_vector, x_ws_test_vector)
    else:
        (f1_org, f1_loc, f1_per) = train_and_evaluate_model(model, x_train_vector, y_train_vector, mask_train_vector,
                                                            x_test_vector, y_test_vector, mask_test_vector, label_dict)

    sum_f1_org += f1_org
    sum_f1_loc += f1_loc
    sum_f1_per += f1_per

    print('Di atas adalah Result of fold ', i+1, "/", folds)

avg_f1_org = sum_f1_org / (folds * 1.0)
avg_f1_loc = sum_f1_loc / (folds * 1.0)
avg_f1_per = sum_f1_per / (folds * 1.0)
